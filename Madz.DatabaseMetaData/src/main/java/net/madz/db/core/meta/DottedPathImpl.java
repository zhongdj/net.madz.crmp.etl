 ...
 */
public class DottedPathImpl implements Iterable<DottedPath>, DottedPath {

    private final DottedPathImpl parent;
    private final String name, absoluteName;

    private DottedPathImpl(DottedPathImpl parent, String name) {
        this.parent = parent;
        this.name = name;
    ...
    /** Constructor */
    public DottedPathImpl(String name) {
        this(null, name);
    }

    public static DottedPath parse(String path) {
        List<String> segments = Arrays.asList(path.split("\\."));
        DottedPathImpl head = new DottedPathImpl(segments.get(0));
        List<String> tail = segments.subList(1, segments.size());
        return head.append(tail);
    }

    public static DottedPathImpl append(DottedPathImpl parent, String segment) {
        return parent != null ? parent.append(segment) : new DottedPathImpl(segment);
    }

    public DottedPathImpl append(String segment) {
        return new DottedPathImpl(this, segment);
    }
    ...
    public boolean equals(Object obj) {
        return ( obj instanceof DottedPathImpl ) && ( (DottedPathImpl) obj ).absoluteName.equals(this.absoluteName);
    }
    ...
    public List<String> toSegmentList() {
        ...
        for ( DottedPathImpl current = this; current != null; current = current.parent ) {
    ...
    /* (non-Javadoc)
     * @see net.madz.db.core.meta.DottedPath#getParent()
     */
    @Override
    ...
    /* (non-Javadoc)
     * @see net.madz.db.core.meta.DottedPath#getAbsoluteName()
     */
    @Override
    ...
    /* (non-Javadoc)
     * @see net.madz.db.core.meta.DottedPath#getName()
     */
    @Override
    ...
    public StringBuilder toString(StringBuilder sb, final String separator) {
        if ( null == sb ) {
            int size = 0;
            for ( DottedPathImpl path = this; path != null; path = path.parent ) {
                if ( path != this ) {
